...
*/
    ...
    // Parameter = scale, 1 is default, 0.5 is half size etc.
    scoreRendererPiste1.init(0.8);
...
function existsSelection() {
    // There is no selection if one of the start/end x value is -1
    // or if both values are the same
    var noSelection = 
        ((selectionForLoop.xStart === -1) || (selectionForLoop.xEnd === -1))
...
function initLoopABListeners() {
    ...
    */
    $("#" + View.frontCanvas.id).mouseup(function(event){
        ...
        if(existsSelection()) {
            ...
            if(start !== -1) {
                jumpTo(start);
                currentSong.loopMode = true;
                $("#loopOnOff").addClass("activated");
                console.log("LoopMode : " + currentSong.loopMode);   
                start == -1;
            }
        } else {
            console.log("No selection : mouse click on canvas, let's jump to another position in the song");
            var mousePos = getMousePos(window.View.frontCanvas, event);
            // will compute time from mouse pos and start playing from there...
            jumpTo(mousePos.x);
...
function resetAllBeforeLoadingANewSong() {
    ...
    // hide music scores from previous song if exists
    $(".alphaTabSurface").hide();
}
...
function drawTrack(decodedBuffer, trackNumber) {
    ...
    var tempCanvasCtx = temporaryCanvas.getContext('2d');

    temporaryCanvas.width = View.masterCanvas.width;
    temporaryCanvas.height = View.masterCanvas.height/currentSong.tracks.length;
    ...
    waveformDrawer.init(decodedBuffer, temporaryCanvas, '#83E83E');
    var x = 0;
    ...
    waveformDrawer.drawWave(0, SAMPLE_HEIGHT);

      tempCanvasCtx.strokeStyle = "white";
      tempCanvasCtx.strokeRect(0, 0, temporaryCanvas.width, SAMPLE_HEIGHT);

      tempCanvasCtx.font = '14pt Arial';
      tempCanvasCtx.fillStyle = 'white';
      tempCanvasCtx.fillText(trackName, 10, 20);

    View.masterCanvasContext.drawImage(temporaryCanvas, 0, 0, temporaryCanvas.width, temporaryCanvas.height,
        0, y, temporaryCanvas.width, temporaryCanvas.height);

    currentSong.tracks[trackNumber].dataURL = temporaryCanvas.toDataURL();
}
...
function finishedLoading(bufferList) {
    ...
    // Display score, first 4 bars at loading time
    if((currentSong.metadata.tabFileName !== undefined) && (currentSong.metadata.tabFileName !== null)){
        scoreRendererPiste1.loadFileAndDisplayScore(currentSong.metadata.tabFileName, 1, 4, View.app.clientWidth);
        $(".alphaTabSurface").show();
    } 
}
...
function animateTime() {
    ...
    if(currentSong !== undefined) {
        ...
        if (!currentSong.paused) {
            ...
            // Schedule sync with score
            // Shloud be called every 1/10s no more... !!!
            // Only if there are metadata defined for this song
            if(currentSong.metadata.sections.length !== 0) {
               currentSection = currentSong.metadata.findScoreSection(currentSong.elapsedTimeSinceStart);

               if(currentSection != undefined) {
                drawScoreInfo(currentSection);
                    if(currentSection !== previousSection) {
                        console.log("showing bars " + currentSection.startBar + " - " + currentSection.endBar + " name " + currentSection.name);
                        scoreRendererPiste1.showBars(currentSection.startBar, currentSection.endBar, View.app.clientWidth);
...
function drawScoreInfo(section) {
    var scoreCanvas = scoreRendererPiste1.container.data('alphaTab').canvas;
    var scoreContext = scoreCanvas.getContext('2d');
    var x = scoreCanvas.width / 2;
    var y = 50;

    scoreContext.clearRect(0, 50, scoreCanvas.width, 40);
    scoreContext.textAlign = 'center';
    scoreContext.font="20px Verdana";
    scoreContext.fillText(section.name + " bar : " + section.findCurrentBar(currentSong.elapsedTimeSinceStart), x, y);
}
