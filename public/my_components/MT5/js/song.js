function Song(songName, context) {
    ...
    this.name = songName;
    // url of this song
    this.url = "MT5/track/" + songName;
    // list of tracks in that song
    this.tracks = [];
    
    // metadata
    this.metadata = new SongMetadata();
    //this.metadata.makeTestSongMetadata();
   ...
   this.addTrack = function(instrument) {
        var currentTrack = new Track(this.name, instrument); 
        this.tracks.push(currentTrack);

        // create a gain for the current track
        this.trackVolumeNodes.push(context.createGain());
   }
